                    ...
                    <TableBody>
                        {items.length === 0 ? (
                            ...
                            items.map((item) => {
                                ...
                                const isItemGenerating = (isGenerating && isGenerating(item.id)) || (item.status?.toUpperCase() === 'GENERATING') || (item.refImageUrl ? (item.refImageUrl.startsWith('TASK:') || item.refImageUrl.toLowerCase().includes('waiting')) : false);
