            ...
            <div className="relative w-[90vw] max-w-5xl flex flex-col gap-4" onClick={e => e.stopPropagation()}>
                ...
                <div className="relative aspect-video bg-black border border-zinc-800 shadow-2xl rounded-lg overflow-hidden group/player">
                    ...
                    <div className="absolute top-4 right-4 z-50 flex gap-2">
                        {/* Sideload to Reference (Images Only) */}
                        {isImage && playingClip && onUpdateClip && (
                            <TooltipProvider>
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <Button
                                            ...
                                            onClick={async (e) => {
                                                ...
                                                const currentRefs = (playingClip.explicitRefUrls || playingClip.refImageUrls || '').split(',').map(s => s.trim()).filter(Boolean);

                                                if (currentUrl && !currentRefs.includes(currentUrl)) {
                                                    const next = [...currentRefs, currentUrl].join(',');
                                                    await onUpdateClip(playingClip.id, {
                                                        refImageUrls: next,
                                                        explicitRefUrls: next
                                                    });
                                                    // Optional: Feedback?
                                                    // For now, the button click is immediate.
                                                    onClose(); // Close after adding, similar to Modal behavior? Or keep open? Modal closed, let's close to show result.
                                                } else {
                                                    alert("This image is already a reference.");
                                                }
                                            }}
                                            className="text-blue-500 hover:text-blue-400 hover:bg-black/20"
                                        >
