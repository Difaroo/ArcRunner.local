...
export function SeriesView() {
    const {
        ...
        libraryItems,
        updateSeries,
        navigateToEpisode // Assuming we have or will implement this in useDataStore wrapper inside page.tsx
    } = useDataStore(s => s)
    ...
    // Derived States
    const [videoPromptTemplate, setVideoPromptTemplate] = useState(DEFAULT_VIDEO_PROMPT)
    ...
    clips.forEach(clip => {
        // filter out other series just in case
        if (clip.series !== currentSeriesId) return;
    ...
    const clipEpKeys = Array.from(progressMap.keys())
    const allEpKeys = new Set([...clipEpKeys, ...seriesEpisodes.map(e => e.id)])

    const sortedEpisodes = Array.from(allEpKeys).sort((a, b) => {
        const numA = parseInt(a.replace(/\D/g, '')) || 0;
        const numB = parseInt(b.replace(/\D/g, '')) || 0;
        return numA - numB;
    }).map(key => {
        const found = seriesEpisodes.find(e => e.id === key)
        return found || { id: key, title: `Episode ${key}`, series: currentSeriesId }
    })
                                ...
                                {sortedEpisodes.map(ep => {
                                    ...
                                    return (
                                        <TableRow
                                            ...
                                            onClick={() => {
                                                if (!isEditing && navigateToEpisode) navigateToEpisode(currentSeriesId, ep.id)
                                            }}
                                        >
                            ...
                            <div className="flex flex-col gap-2 shrink-0">
                                ...
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (currentSeriesId && updateSeries) {
                                            updateSeries(currentSeriesId, { defaultModel: newModel })
                                        }
                                    }}
